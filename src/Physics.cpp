...
// Camera
bool Physics::addCamera(Camera* camera) {
	// Check if entity have any physical representation.
	if (
		camera->pCollisionActor == NULL ||
		camera->pCollisionShape == NULL
		) {
		return false;
	}

	// Create physical material
	camera->pCollisionShape->pMaterial = this->pxPhysics->createMaterial(1, 1, 0);

	// Create shape
	camera->pCollisionShape->pShape = this->pxPhysics->createShape(
		*(camera->pCollisionShape->pGeometry),
		*(camera->pCollisionShape->pMaterial)
	);

	// Initial transform
	PxTransform rigidActorTransform = camera->gPosition.toPxTransform();

	// Physical actor of the camera
	PxActor* rigidActor = NULL;

	// Kinematic actor
	PxRigidDynamic* rigidDynamicActor = this->pxPhysics->createRigidDynamic(rigidActorTransform);
	rigidDynamicActor->attachShape(*(camera->pCollisionShape->pShape));
	rigidDynamicActor->setRigidBodyFlag(PxRigidBodyFlag::eKINEMATIC, true);
	...
	// Update rigid actor pointer.
	camera->pCollisionActor->pActor = rigidActor;

	// Add actor to scene.
	this->pxScene->addActor(*(camera->pCollisionActor->pActor));

	return true;
}

void Physics::updateCamera(Camera* camera) {
	// Check if camera have any physical representation.
	if (camera->pCollisionActor == NULL) {
		return;
	}

	// Initial transform
	PxTransform tm = PxTransform(camera->gPosition.toPxVec3());

	// Kinematic
	// Get rigid object.
	PxRigidDynamic* rigidDynamic = camera->pCollisionActor->pActor->is<PxRigidDynamic>();
	if (rigidDynamic == NULL) {
		return;
	}
	...
	// Integrate entities' physics position and rotation with graphics side.
	camera->gPosition.x = tm.p.x;
	camera->gPosition.y = tm.p.y;
	camera->gPosition.z = tm.p.z;

	camera->dataChanged = true;
}
